...
function showSlashHelp() {
  addMessage("assistant", [
    ...
    "- `/ro [request]` — reference files in LLM_READ_ONLY_FILES (no request lists them)",
    "- `/rw [request]` — reference files in LLM_READ_WRITE_FILES (no request lists them)",
...
function handleSlash(text) {
  ...
  switch (cmd) {
    ...
    case "ro": case "readonly": {
      const base = "Using the files in /LLM_READ_ONLY_FILES (list_dir it, then read_file / read_document / analyze_image the relevant ones — my READ-ONLY shared folder), ";
      send(arg ? base + arg : base + "list what's in it and briefly say what each file is.");
      return;
    }
    case "rw": case "readwrite": {
      const base = "Using the files in /LLM_READ_WRITE_FILES (list_dir it, then read the relevant ones — my READ-WRITE shared folder; my uploads land under uploads/ and your deliverables are saved here), ";
      send(arg ? base + arg : base + "list what's in it and briefly say what each file is.");
      return;
    }
