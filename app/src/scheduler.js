...
async function runTask(task) {
  ...
  sys += " Based on what you find, take whatever actions are appropriate using your tools. OUTPUT DESTINATIONS — send results wherever the task asks (default to what the user's instructions say): post_to_chat(message) writes a message straight into the user's LIVE CHAT conversation window (use this whenever the task should show its output in the chat / talk to the user); notify_user(message) is a passive alert/badge (and it STOPS a recurring task, so only use it as the stop signal or for one-shots); append_log/write_file to /LLM_READ_WRITE_FILES is a persistent file/log. You can combine them (e.g. log to a file AND post_to_chat). If the user asked for the result in the conversation, you MUST call post_to_chat — do not just put it in your final text (that goes only to the task record, not the chat).";
